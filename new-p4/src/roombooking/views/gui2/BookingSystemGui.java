...
public class BookingSystemGui
    ...
    private void setupFrame() {
        ...
        // want system to keep running until user also quits CLI
    ...
    private void setupInfo() {
        ...
        String usageIntructions = """
                <html>
                <b>How to use:</b>
                1. (Optional) Load in data from a previous session<br>
                2. View Bookings. Click on column headers to sort.<br>
                3. Add and Remove Bookings, People, Buldings, and Rooms.<br>
                4. Save your session (to a JSON file).<br>
                <br>
                Note:
                """;
        // ImageIcon infoIcon = new ImageIcon("../../assets/imgs/info_icon.png");
        // JLabel useInfo = new JLabel(usageIntructions);
        Icon icon = new ImageIcon("data/imgs/icon.png");
        JLabel nameInfo = new JLabel(usageIntructions, icon, JLabel.LEADING);
        // new JLabel(icon, "Room Booking System");
        // new JLabel
        // Find out how much the font can grow in width.
        // double widthRatio = (double) componentWidth / (double) stringWidth;

        // int newFontSize = (int) (labelFont.getSize() * widthRatio);
        // nameInfo.setFont(new Font(nameInfo.getName(), Font.PLAIN,
        // Math.min(newFontSize, infoPanel.getHeight())));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        PopUpFrame frameHelper = new PopUpFrame(model);
        if (e.getSource() == loadButton) {
            loadSystem();
        } else if (e.getSource() == saveButton) {
            saveSystem();
        } else if (e.getSource() == newBooking) {
            frameHelper.newBooking(controller);
        } else if (e.getSource() == newPerson) {
            frameHelper.newPerson(controller);
        } else if (e.getSource() == newBuilding) {
            frameHelper.newBuilding(controller);
        } else if (e.getSource() == newRoom) {
            frameHelper.newRoom(controller);
        } else if (e.getSource() == removeRoom) {
            frameHelper.removeRoom(controller);
        } else if (e.getSource() == removeBooking) {
            frameHelper.removeBooking(controller);
        } else if (e.getSource() == removeBuilding) {
            frameHelper.removeBuilding(controller);
        } else if (e.getSource() == removePerson) {
            frameHelper.removePerson(controller);
        }
    }
